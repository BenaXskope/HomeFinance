...
def fillUp(apps, schema_editor):
    ...
    for t in range(100):
        ...
        cat = Category(title='Еда', color='#ffffff', account=ac, prognosis=prognose)
        cat.save()
        ...
        prognose = random.randint(1, 50) * 1000
        cat = Category(title='Транспорт', color='#ffffff', account=ac, prognosis=prognose)
        cat.save()
        catPred = CategoryPrognosis(categoryId=cat, value=prognose)
        catPred.save()
        for r in range(100):
        ...
        prognose = random.randint(1, 50) * 1000
        cat = Category(title='Технкиа', color='#ffffff', account=ac, prognosis=prognose)
        cat.save()
        catPred = CategoryPrognosis(categoryId=cat, value=prognose)
        catPred.save()
        for r in range(100):
        ...
        prognose = random.randint(1, 50) * 1000
        cat = Category(title='Непрдвиденные расходы', color='#ffffff', account=ac, prognosis=prognose)
        cat.save()
        catPred = CategoryPrognosis(categoryId=cat, value=prognose)
        catPred.save()
        for r in range(100):
